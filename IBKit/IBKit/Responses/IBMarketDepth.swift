...
public struct IBMarketDepth: IBAnyMarketDepth {
    ...
    public init(from decoder: IBDecoder) throws {
        ...
        position = try container.decode(Int.self)
        operation = try container.decode(Int.self)
        side = try container.decode(Int.self)
...
public struct IBMarketDepthLevel2: IBAnyMarketDepth {
    ...
    public init(from decoder: IBDecoder) throws {
        ...
        requestID = try container.decode(Int.self)
        position = try container.decode(Int.self)
        marketMaker = try container.decode(String.self)
        operation = try container.decode(Int.self)
        side = try container.decode(Int.self)
        price = try container.decode(Double.self)
        size = try container.decode(Double.self)
        isSmart = serverVersion >= IBServerVersion.SMART_DEPTH ? try container.decode(Bool.self) : false
    }
}
